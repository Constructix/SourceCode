...
{ 
    ...
    class Program
    {
        ...
        private const string keyPath = @"D:\Downloads\TextRecognition\20181231_123550.jpg";
        private static readonly RegionEndpoint bucketRegion =  RegionEndpoint.APSoutheast2;
        private static  IAmazonS3 client;
        public static async Task Main(string[] args)
        {
            ProgramHeadingMessage();

            client =  new AmazonS3Client(bucketRegion);
            Console.WriteLine($"Adding {keyPath}");
            await PutObjectIntoBucket();
            Console.Read();
        }

        static async Task PutObjectIntoBucket()
        {
            try
            {
                // create object 
        ...
        private static void ProgramHeadingMessage()
        {
            Console.WriteLine(new string('-', 80));
            Console.WriteLine(" Adding an image to s3 Object.");
            Console.WriteLine(new string('-', 80));
